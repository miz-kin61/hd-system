...
def get_dictionaries(mode):
    if "ビジネス" in mode:
        ...
        center_issues = {
            ...
            "生命力": {"curse": "🌀 【思い込み】「他人のペースに引きずられ、疲れ果ててしまう」", "truth": "✨ 【本来の強み】必要な分だけ活力を出して、用が済んだらすっと手放せる「充電不要な電池」。", "solution": "💡 【整える言葉】「今日分の活力は使い切った。体が止まれと言う前に、先に休む」"},
...
if 'report_data' in st.session_state:
    ...
    for i in range(rd['total_score']):
        bg = "#E53935" if i < current_score else "#CFD8DC" # 色を赤に変更
...
display_mode = st.sidebar.radio(
    "誰に診断結果を見せますか？", 
    ["🌿 やさしい言葉（初心者・一般）", "🏫 中高生向け（学園生活・青春）", "💻 理系・システム工学（論理派）"],
    on_change=reset_state
)
...
def generate_report_data(data, jd_d, y, m, d, h, mi, mode):
    ...
    with contextlib.redirect_stdout(exp_f):
        ...
        # ▼ ミッシングゲートの表示強化！
        if len(initial_islands) <= 1:
            print("🌟 ミッシングゲート（橋渡しの扉）: なし（既に全体が繋がっています）\n")
        else:
            if b_gates_1:
                print(f"🌟 ミッシングゲート（島を繋ぐ橋渡しの扉）:\n")
                for bg in sorted(list(set(b_gates_1))):
                    mean_str = GATE_TECH_MEANINGS.get(bg, "")
                    print(f"  └ 扉 {bg:>2}: {mean_str}\n")
            elif b_gates_2:
                print(f"🌟 広域ミッシングゲート（2つの扉で繋がる）:\n")
                for bg1, bg2 in b_gates_2:
                    print(f"  └ [扉 {bg1} & 扉 {bg2}]\n")
...
if 'report_data' in st.session_state:
    ...
    # 🌟 鍵付き！VIP専用の専門家エリア
    with st.expander("▼ 🔒【専門データ】ゲート・ライン・天体の詳細を開く（※要パスコード）"):
        st.info("💡 ここから先は専門家（アナリスト）向けの生データエリアです。")
        
        # パスワード入力欄（type="password" で文字が ●●● になります）
        secret_code = st.text_input("公式LINEで配布している4桁のパスコードを入力してください：", type="password")
        
        # ▼ ここがパスワードの正解です！（好きな文字・数字に変更できます）
        if secret_code == "2026":
            st.success("認証成功！専門データを展開します。")
            st.markdown(f"<div class='card' style='background-color:#f8f9fa;'>\n{rd['html_expert']}\n</div>", unsafe_allow_html=True)
        elif secret_code != "":
