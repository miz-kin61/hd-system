...
DEFINED_CENTER_ISSUES = {
    "Head": {
        ...
        "truth": "【本来の仕様】自分に必要なデータだけをピンポイントで受信する『専用クローズドアンテナ』。",
        "solution": "【推奨デバッグコマンド】「私のシステムに他人のデータを混ぜない。興味のないことは完全シャットアウト」"
    },
    "Ajna": {
        "curse": "【インストールされた外部バグ】「人の意見に合わせて柔軟になれ」「頑固は悪だ」\n └ 自分の確固たる思考プロセスを自己否定し、無理に他人の意見をマージ（統合）しようとして論理破綻する。",
        "truth": "【本来の仕様】固定された確実なプロセスで情報を処理する『確固たる情報処理モジュール』。",
        "solution": "【推奨デバッグコマンド】「私の論理回路は固定仕様。他人に同調せず、ただ私の見解として出力すればいい」"
    },
    "Throat": {
        "curse": "【インストールされた外部バグ】「もっと言葉を選べ」「言い方がキツい」\n └ 安定した出力ポートを無理に制御しようとして、システム全体にストレスがかかる。",
        "truth": "【本来の仕様】自分のスタイルで確実にアウトプットを生成する『専用出力インターフェース』。",
        "solution": "【推奨デバッグコマンド】「出力フォーマットは変更不可。これが私の標準出力（デフォルト）です」"
    },
    "G": {
        "curse": "【インストールされた外部バグ】「環境や相手に合わせてカメレオンになれ」「協調性を持て」\n └ 本来固定されている自分のスタンス（IPアドレス）を無理に変えようとして、アイデンティティが迷子になる。",
        "truth": "【本来の仕様】自分が中心となり、正しい環境と人を引き寄せる『固定ルーティング・サーバー』。",
        "solution": "【推奨デバッグコマンド】「私は動かない。私というサーバーに合うクライアント（人）だけがアクセスすればいい」"
    },
    "Heart": {
        "curse": "【インストールされた外部バグ】「謙虚であれ」「お金や評価を要求するのははしたない」\n └ 自分の価値を安売りし、正当な対価をもらわずにリソースを提供してしまい、基盤がすり減る。",
        "truth": "【本来の仕様】確固たる意志力で取引を完遂し、正当な対価を受け取る権利がある『エンタープライズ版モジュール』。",
        "solution": "【推奨デバッグコマンド】「安売りはシステム劣化の元。自分の価値を認め、堂々と対価（お金や評価）を要求する」"
    },
    "Sacral": {
        "curse": "【インストールされた外部バグ】「頭で考えて計画的に動け」「自分から発信しろ」\n └ 外部のPing（フック）を待たずに思考で無理やり処理を開始し、空回りして徒労感（エラー）を溜め込む。",
        "truth": "【本来の仕様】外部からの問いかけに『応答』した時にだけ、無限の動力が起動する『超・レスポンス型エンジン』。",
        "solution": "【推奨デバッグコマンド】「頭で考えて動かない。肚（システム深部）が反応するまで、ただ待機（リスン）する」"
    },
    "Splenic": {
        "curse": "【インストールされた外部バグ】「直感だけで動くな」「論理的に、エビデンス（証拠）を出せ」\n └ 瞬間の『野生の警告』を無視して思考で理由を探し、結局タイミングを逃して危険な目に遭う。",
        "truth": "【本来の仕様】今、この瞬間の生存（正解）を理由なく一瞬で知らせる『超高速・危機回避アラート』。",
        "solution": "【推奨デバッグコマンド】「理由は分からないけど、なんか嫌だからそうする。エビデンス（証拠ログ）は不要」"
    },
    "SolarPlexus": {
        "curse": "【インストールされた外部バグ】「即断即決しろ！」「チャンスを逃すな、今すぐ決めろ！」\n └ 焦って感情の波の「高揚」や「どん底」の瞬間にノリで決断し、後になって大後悔（致命的エラー）を起こす。",
        "truth": "【本来の仕様】時間をかけるほどに真実の解像度が上がる『熟成型・絶対権威モジュール』。",
        "solution": "【推奨デバッグコマンド】「素晴らしい提案ですね。一旦キャッシュに保存して、数日寝かせてからお返事します」"
    },
    "Root": {
        "curse": "【インストールされた外部バグ】「自分のペースでゆっくりやれ」「焦らなくていい」\n └ 本来持っている「プレッシャーを動力に変換する機能」を抑え込まれ、本来の爆発力が出せない。",
        "truth": "【本来の仕様】自らプレッシャーを生み出し、アドレナリンと共に一気にタスクを消化する『バッチ処理型パルスエンジン』。",
        "solution": "【推奨デバッグコマンド】「私はプレッシャーがある方が燃える。ギリギリのバッチ処理こそ私の最適稼働」"
    }
}
...
PROFILE_TECH_MEANINGS = {
    ...
    "2/4": "【クローズド・エコシステム】天性のブラックボックス・アルゴリズム（2）を持ち、信頼できるネットワーク（4）内だけでその才能を開花させる。",
    "2/5": "【特命デバッガー】天才的な直感コード（2）を持ち、外部からのSOS（5）に応えて颯爽とソリューションを提供する。",
    "3/5": "【トラブルシューター】エラーと衝突（3）から実践的な最適解を見つけ出し、社会のバグを修正する（5）問題解決のプロ。",
    "3/6": "【フェーズ移行型アーキテクト】前半生はバグ出し（3）に奔走し、後にその経験を活かして全体を俯瞰する設計者（6）へ進化する。",
    "4/6": "【コミュニティ・モデレーター】人脈ネットワーク（4）を構築しつつ、高い視座（6）から組織システムの健全性を監査する。",
    "4/1": "【エバンジェリスト】強固な人脈（4）をベースに、揺るぎない基礎知識（1）を布教していく伝道師。",
    "5/1": "【CTO / 技術顧問】圧倒的な基礎知識（1）を土台に、クライアントの期待（5）に完璧なソリューションで応える最高技術責任者。",
    "5/2": "【凄腕ハッカー】普段は隠れている（2）が、周囲の過剰な期待（5）に対して天才的な一撃で問題を解決する職人。",
    "6/2": "【天上人アーキテクト】天性の才能（2）を持ちつつ、現場のコード書きからは離れ、システム全体を俯瞰する（6）システム監査役。",
    "6/3": "【客観的テスター】自らエラーに飛び込み（3）つつも、どこか俯瞰した視点（6）でシステムの限界をテストし続ける探求者。"
}
