...
def print_master_report(data, jd_d, y, m, d, h, mi):
    ...
    print(f"<span class='unconscious-red'>身体（先天）: {dj.strftime('%Y/%m/%d %H:%M')} (日本時間)</span>\n")
    ...
    if len(initial_islands) == 0:
        ...
        definition_type = "四系統型（クアッド・スプリット）"
    else:
...
def print_tech_spec_report(type_str, on_centers, gates, data, def_type, b_gates_1, b_gates_2, islands):
    ...
    print("\n【層別の構成（各層の扉の数）】")
    print("層 | <span class='unconscious-red'>先天</span> | 後天 | 合計 | 特性")
    print("-" * 35)
    for i in range(1, 7):
...
# タイトル: HD自作エンジン Webアプリ版 (Ver 3.5.2 表示最適化・年齢制限解除版)
# =====================================================================
import streamlit as st
import io
import contextlib
...
# 🌟修正ポイント：高齢者の方でも入力できるように1920年〜今日までに制限を解除！
input_date = st.sidebar.date_input(
    "生年月日", 
    value=datetime.date(1980, 1, 1),
    min_value=datetime.date(1920, 1, 1),
...
def print_master_report(data, jd_d, y, m, d, h, mi):
    ...
    # 🌟修正ポイント：身体の中枢の直前に「表現と連動している〜」を移動
    if connected_motors:
        motors_jp = "・".join(connected_motors)
        print(DIVIDER)
        print(f"💡 表現と連動している活力の源: {motors_jp}")
